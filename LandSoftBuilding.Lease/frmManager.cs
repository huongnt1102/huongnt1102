    ...
    public partial class frmManager : DevExpress.XtraEditors.XtraForm
    {
        ...
        private void Delete()
        {
            ...
            try
            {
                ...
                {
                    var objHD = db.ctHopDongs.FirstOrDefault(p => p.ID == (int)gvHopDong.GetRowCellValue(i, "ID"));
                    if (objHD == null) continue;
        ...
        private void linqInstantFeedbackSource1_GetQueryable(object sender, DevExpress.Data.Linq.GetQueryableEventArgs e)
        {
            ...
            var sql = from p in db.ctHopDongs
                ...
                where p.MaTN == (byte?) itemToaNha.EditValue & p.IsHopDongTOS.GetValueOrDefault() == false
                ...
                using (var db = new MasterDataContext())
                {
                    gcChiTiet_PhuLuc.DataSource = from p in db.ctPhuLuc_ChiTiets
                                                  ...
                                                  select new
                                                  {
                                                      ...
                                                      p.TuNgay,
                                                      p.DenNgay,
                                                      TenLoaiGiamGia = p.MaLoaiGiamGia == 1 ? "Giảm theo tỉ lệ %" : (p.MaLoaiGiamGia == 2 ? "Giảm theo số tiền" : ""),
                                                      TenLoaiDichVu = p.MaLDV == 1 ? "Giá thuê" : (p.MaLDV == 2? "Phí quản lý" : (p.MaLDV == 3 ? "Phí điều hòa chiếu sáng": "")), 
                                                      p.SoTienGiam,
                                                      p.PhanTramGiamGia,
                                                      p.Coc_SoTien,
        ...
        private void itemChuyenTienCocVeThuTruoc_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            var id = (int?)gvHopDong.GetFocusedRowCellValue("ID");
            if (id == null) return;
            if (id == null)
            ...
            using (var frm = new frmChuyenTienCocVeThuTruoc() { MaTN = (byte?)itemToaNha.EditValue, MaHD = id })
            {
                frm.ShowDialog();
                if (frm.DialogResult == DialogResult.OK)
                {
