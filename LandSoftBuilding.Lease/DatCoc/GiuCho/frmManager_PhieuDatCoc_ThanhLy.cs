...
{
    public partial class frmManager_PhieuDatCoc_ThanhLy : DevExpress.XtraEditors.XtraForm
    {
        public frmManager_PhieuDatCoc_ThanhLy()
        {
            InitializeComponent();
        }
            ...
            using (var db = new MasterDataContext())
            {
                var sql = (from pdc in db.PhieuDatCoc_GiuChos
                           ...
                           where
                                 ...
                                 & (//pdc.MaTT == 2 // Đã duyệt
                                 //|| 
                                 pdc.MaTT == 9 // Đã thu cọc
                                 //|| pdc.MaTT == 6 // Đã lên hợp đồng
                                 //|| pdc.MaTT == 7 //Đã hoàn trả tiền cọc
                                 || pdc.MaTT == 8
                                 )
                           select new
                           {
                               pdc.ID,
                               pdc.SoCT,
                               TrangThai = (pdc.MaTT == 2 || pdc.MaTT == 3) ? "Chờ xử lý" : tt.TrangThai,
        ...
        private void barButtonItem7_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            if (id != null)
            {
                var id_TrangThai = Convert.ToInt32(gvHopDong.GetFocusedRowCellValue("MaTT"));
                var TrangThaiTT = 9;
                using (frmCapNhatTrangThai frm = new frmCapNhatTrangThai(Convert.ToInt32(id), TrangThaiTT))
                {
                    frm.ShowDialog();
                    LoadData();
                }
        ...
        private void barButtonItem4_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            try
            {
                ...
                if (id != null)
                {
                    ...
                    //if(id_TrangThai != 8)
                    //{
                    //    DialogBox.Alert("Vui lòng duyệt thanh lý trước. Xin cảm ơn!");
                    //    return;
                    //}
                    using (frmEdit frm = new frmEdit { MaTN = (byte?)itemToaNha.EditValue })
                    {
