...
{
    public partial class frmEditMoi : DevExpress.XtraEditors.XtraForm
    {
        ...
        void TinhThanhTien()
        {
            try
            {
                ...
                spTienTT.Value = Math.Round((decimal)(spinThanhTien.Value / (decimal)1.1), 0, MidpointRounding.AwayFromZero);
                spTienVAT.EditValue = Math.Round((decimal)spinThanhTien.Value - spTienTT.Value, 0, MidpointRounding.AwayFromZero);
                spPhiDV.Value = Convert.ToDecimal(spTienTT.EditValue) / spKyTT.Value;
                spinTienCK.EditValue = Math.Round((decimal)(spTienTT.Value * spinTyLeCK.Value), 0, MidpointRounding.AwayFromZero);
