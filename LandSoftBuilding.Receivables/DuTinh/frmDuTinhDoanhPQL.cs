...
{
    public partial class frmDuTinhDoanhPQL : DevExpress.XtraEditors.XtraForm
    {
        public frmDuTinhDoanhPQL()
        {
            InitializeComponent();
        }
        ...
        void LoadData()
        {
            ...
            try
            {
                ...
                var objDuTinh = (from hd in db.dvHoaDons
                                 join dv in db.dvDichVuKhacs on new { TableName = hd.TableName, LinkID = hd.LinkID } equals new { TableName = "dvDichVuKhac", LinkID = (int?)dv.ID }
                                 where ltToaNha.Contains(hd.MaTN)
                                  & !dv.IsNgungSuDung.GetValueOrDefault()
                                  & hd.NgayTT.Value.Month == thang
                                  & hd.NgayTT.Value.Year == nam
                                  & hd.MaLDV == 13
                                  & hd.IsDuyet == true  //& hd.MaKH == 3099
