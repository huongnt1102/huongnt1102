...
{
    public partial class ctlThongKeYeuCauGuiDenPhongBan : DevExpress.XtraEditors.XtraUserControl
    {
        tnNhanVien objnhanvien;
        public ctlThongKeYeuCauGuiDenPhongBan(tnNhanVien obj)
        {
            InitializeComponent();
            objnhanvien = obj;
            LoadKBC();
        ...
        void LoadData()
        {
            if (itemTuNgay.EditValue != null & itemDenNgay.EditValue != null)
            {
                ...
                sr.DataSource = LayDSYeuCau(tuNgay, denNgay, objnhanvien);
                ...
                title.Text = "Thống kê yêu cầu gửi đến các bộ phận";
        ...
        private List<ThongKe> LayDSYeuCau(DateTime TuNgay, DateTime DenNgay, tnNhanVien objnhanvien)
        {
            List<ThongKe> lstThongKe = new List<ThongKe>();

            using (MasterDataContext db = new MasterDataContext())
            {
                var lstTT = db.tnPhongBans.ToList();
                foreach (var objtt in lstTT)
                {
                    if (objnhanvien.IsSuperAdmin.Value)
                    {
                        ...
                        {
                            TrangThai = objtt.TenPB,
                            SoLuong = db.tnycYeuCaus
                            .Where(p => p.MaBP == objtt.MaPB
