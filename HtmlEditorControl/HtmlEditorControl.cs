...
namespace MSDN.Html.Editor
{
    ...
    public sealed partial class HtmlEditorControl : UserControl
    {
        ...
        [Category("ImageBrowser"), Description("A Image Browser Event was encountered")]
        public event ImageBrowserEventHandler ImageBrowser;
        [Category("FileBrowser"), Description("A File Browser Event was encountered")]
        public event FileBrowserEventHandler FileBrowser;
        ...
        private void btnBrowser_Click(object sender, EventArgs e)
        {
            ...
            {
                ImageBrowserEventArgs imageBrowserEventArgs = new ImageBrowserEventArgs();
                this.ImageBrowser(this, imageBrowserEventArgs);
                if (imageBrowserEventArgs.ImageUrl != "")
                {
                    Button button = (Button)sender;
                    Form form = (Form)button.Parent;
                    form.Controls["hrefLink"].Text = imageBrowserEventArgs.ImageUrl;
                    form.Controls["hrefLink"].Focus();
                }
            }
        }
        ...
        private void btnFileBrowser_Click(object sender, EventArgs e)
        {
            ...
            {
                FileBrowserEventArgs fileBrowserEventArgs = new FileBrowserEventArgs();
                this.FileBrowser(this, fileBrowserEventArgs);
                if (fileBrowserEventArgs.FileUrl != "")
                {
                    Button button = (Button)sender;
                    Form form = (Form)button.Parent;
                    form.Controls["hrefLink"].Text = fileBrowserEventArgs.FileUrl;
                    form.Controls["hrefLink"].Focus();
                }
            }
        }
        ...
        public void InsertImagePrompt()
        {
            ...
            int width = 0;
            int height = 0;
            ImageAlignOption imageAlignOption = ImageAlignOption.Left;
            mshtmlElement firstControl = this.GetFirstControl();
            if (firstControl != null)
            {
                if (!this.IsStringEqual(firstControl.tagName, "IMG"))
                {
                    throw new HtmlEditorException("Can only Insert an Image over a previous Image.", "InsertImage");
                }
                mshtmlImageElement iHTMLImgElement = (mshtmlImageElement)firstControl;
                text2 = iHTMLImgElement.href;
                text = iHTMLImgElement.alt;
                width = iHTMLImgElement.width;
                height = iHTMLImgElement.height;
                if (iHTMLImgElement.align != null)
                {
                    imageAlignOption = (ImageAlignOption)this.TryParseEnum(typeof(ImageAlignOption), iHTMLImgElement.align, ImageAlignOption.Left);
                }
            }
            using (EnterImageForm enterImageForm = new EnterImageForm())
            {
                enterImageForm.ImageLink = text2;
                enterImageForm.ImageText = text;

                enterImageForm.WidthImage = width;
                enterImageForm.HeightImage = height;

                enterImageForm.ImageAlign = imageAlignOption;
                enterImageForm.Controls["btnBrowser"].Click += new EventHandler(this.btnBrowser_Click);
                this.DefineDialogProperties(enterImageForm);
                if (enterImageForm.ShowDialog(this.ParentForm) == DialogResult.OK)
                {
                    text2 = enterImageForm.ImageLink;
                    text = enterImageForm.ImageText;
                    imageAlignOption = enterImageForm.ImageAlign;
                    if (text2 != string.Empty)
                    {
                        this.ExecuteCommandRange("InsertImage", text2);
                        firstControl = this.GetFirstControl();
                        if (firstControl != null)
                        {
                            ...
                            {
                                mshtmlImageElement iHTMLImgElement = (mshtmlImageElement)firstControl;
                                iHTMLImgElement.alt = text;
                                if (imageAlignOption != ImageAlignOption.Left)
                                {
                                    iHTMLImgElement.align = imageAlignOption.ToString().ToLower();
                                }
                                if (enterImageForm.WidthImage > 0)
                                {
                                    iHTMLImgElement.width = enterImageForm.WidthImage;
                                }
                                if (enterImageForm.HeightImage > 0)
                                {
                                    iHTMLImgElement.height = enterImageForm.HeightImage;
                                }
                            }
                        }
                    }
            ...
            //    dialog.Controls["btnBrowser"].Click += new EventHandler(this.btnBrowser_Click);
            //    DefineDialogProperties(dialog);
        ...
        public void InsertLinkPrompt()
        {
            ...
            if (hrefText != null)
            {
                ...
                using (EnterHrefForm dialog = new EnterHrefForm())
                {
                    ...
                    dialog.Controls["btnBrowser"].Click += new EventHandler(this.btnFileBrowser_Click);
                    DefineDialogProperties(dialog);
        ...
        private void toolstripInsertHtml_Click(object sender, EventArgs e)
        {
            //ToolStripButton button = (ToolStripButton)sender;
            //string command = (string)button.Tag;
            //ProcessCommand(command);
