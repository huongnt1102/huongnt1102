...
namespace Building.SMSZalo.NhanVien
{
    public partial class frmNhanVien : Form
    {
        ...
        void LoadData()
        {
            ...
            {
                var tnkhachang = db.tnNhanViens.FirstOrDefault(o => o.IdZalo == item.user_id);
                if (tnkhachang != null)
                    continue;
            ...
            var ltKhachHang = (from kh in db.tnNhanViens
                               select new
                               {
                                   kh.MaTN,
                                   MaKH = kh.MaNV,
            ...
            try
            {
                ...
                foreach (var item in ltdata)
                {
                    ...
                    if (i.MaKH != 0)
                    {
                        try
                        {
                            var objkh = db.tnNhanViens.FirstOrDefault(o => o.MaNV == i.MaKH);
                            objkh.IdZalo = i.user_id;
                ...
                foreach (var item in ltdata)
                {
                    ...
                    if (i.phone == "")
                    {
                        ...
                        JObject result = client.sendRequestUserProfileToUserId(smsid, 
                            ...
                            "Để tiện cho việc gửi tin nhắn thông báo đến nhân viên, nay ban quản lý yêu cầu cư dân cung cấp thông tin số điện thoại. Vui lòng 'Click' vào đây để điền các thông tin. Việc cung cấp này sẽ được bảo mật, xin cảm ơn!",
        ...
        private void itemLuuKhachHangSoDienThoai_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            try
            {
                ...
                foreach (var item in ltdata)
                {
                    ...
                    if (i.phone != "")
                    {
                        var objkh = db.tnNhanViens.FirstOrDefault(o => o.DienThoai == i.phone);
                        if(objkh != null)
                        {
                            objkh.IsZalo = true;
                            objkh.NameZalo = i.display_name;
