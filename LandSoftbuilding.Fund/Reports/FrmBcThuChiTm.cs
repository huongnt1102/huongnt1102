        ...
        private void LoadData()
        {
            ...
            try
            {
                ...
                if (itemTuNgay.EditValue != null && itemDenNgay.EditValue != null)
                {
                    ...
                    //var objDauKy = db.SoQuy_ThuChis.Where(_ => SqlMethods.DateDiffDay(_.NgayPhieu, tuNgay) > 0 & _.MaTN == ((byte?)itemToaNha.EditValue ?? Common.User.MaTN) & _.IsKhauTru == false & _.MaLoaiPhieu!=24)
                    //    .Select(_ => new { SoTien = _.IsPhieuThu == true ? _.DaThu : -_.DaThu }).ToList();
                    //var objDauKy = db.SoQuy_ThuChis.Where(_ => SqlMethods.DateDiffDay(_.NgayPhieu, tuNgay) > 0 & _.MaTN == ((byte?)itemToaNha.EditValue ?? Common.User.MaTN) & _.IsKhauTru == false & _.MaLoaiPhieu != 24)
                    //    .Select(_ => new { SoTien = _.IsPhieuThu == true ? _.DaThu : -_.DaThu }).ToList();
                    //var objPhieuChi = db.SoQuy_ThuChis.Where(_ => SqlMethods.DateDiffDay(_.NgayPhieu, tuNgay) > 0 & _.MaTN == ((byte?)itemToaNha.EditValue ?? Common.User.MaTN) & _.IsKhauTru == false & _.MaLoaiPhieu != 24 & _.IsPhieuThu == false)
                    //    .Select(_ => new { SoTien = _.IsPhieuThu == true ? _.DaThu : -_.DaThu }).ToList();
                    //decimal soTien = objDauKy.Sum(_ => _.SoTien).GetValueOrDefault() - objPhieuChi.Sum(_ => _.SoTien).GetValueOrDefault();
                    var objDauKy = (from _ in db.SoQuy_ThuChis
                                    join pt in db.ptPhieuThus on _.IDPhieu equals pt.ID into dspt
                                    from pt in dspt.DefaultIfEmpty()
                                    where (pt != null ? SqlMethods.DateDiffDay(pt.NgayThu, tuNgay) > 0 : SqlMethods.DateDiffDay(_.NgayPhieu, tuNgay) > 0) & _.MaTN == ((byte?)itemToaNha.EditValue ?? Common.User.MaTN) & _.IsKhauTru == false & _.MaLoaiPhieu != 24 & ((_.IsPhieuThu == true && pt != null && pt.MaTKNH == null) | _.IsPhieuThu == false)
                                    select new { SoTien = _.IsPhieuThu == true ? _.DaThu : -_.DaThu }).ToList();
                    ...
                    var objTrongKy1 = (from _ in db.SoQuy_ThuChis
                                       ...
                                       where (pt != null ? (SqlMethods.DateDiffDay(tuNgay, pt.NgayThu) >= 0 &
                                             SqlMethods.DateDiffDay(pt.NgayThu, denNgay) >= 0) : (SqlMethods.DateDiffDay(tuNgay, _.NgayPhieu) >= 0 &
                                       ...
                                       {
                                           NgayHachToan = pt != null ? pt.NgayThu : _.NgayPhieu,
                    ...
                    var objTrongKys = objTrongKy1.Concat(objTrongKy2).Concat(objTrongKy3).Concat(objTrongKy4).OrderBy(_ => _.NgayHachToan);
