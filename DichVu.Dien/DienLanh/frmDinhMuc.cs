    ...
    public partial class frmDinhMuc : DevExpress.XtraEditors.XtraForm
    {
        ...
        void LoadData()
        {
            try
            {
                ...
                if (_MaMB != null)
                    gcDinhMuc.DataSource = db.dvDienLanhDinhMucs.Where(p => p.MaMB == _MaMB);
                else if (_MaLMB != null)
                    gcDinhMuc.DataSource = db.dvDienLanhDinhMucs.Where(p => p.MaLMB == _MaLMB & p.MaMB == null);
                else
                    gcDinhMuc.DataSource = db.dvDienLanhDinhMucs.Where(p => p.MaTN == _MaTN & p.MaLMB == null & p.MaMB == null);
            }
            catch
            {
        ...
        private void itemToaNha_EditValueChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                lkLoaiDien.DataSource = db.dvDienLoaiDiens.Select(p=> new {p.ID, p.TenLoaiDien}).ToList();
