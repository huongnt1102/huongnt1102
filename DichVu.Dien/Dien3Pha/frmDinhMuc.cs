    ...
    public partial class frmDinhMuc : DevExpress.XtraEditors.XtraForm
    {
        ...
        void LoadData()
        {
            try
            {
                ...
                if (_MaMB == null && _MaLMB == null && _MaKH == null)
                    gcDinhMucNuoc.DataSource = db.dvDien3PhaDinhMucs.Where(p => p.MaTN == maTN && p.MaKH==null && p.MaLMB==null && p.MaMB==null);
                else
                    gcDinhMucNuoc.DataSource = db.dvDien3PhaDinhMucs.Where(p => p.MaTN == maTN && (p.MaLMB == _MaLMB || (p.MaKH == _MaKH & p.MaMB == _MaMB)));
            }
            catch
            {
        ...
        void Delete()
        {
            ...
            try
            {
                ...
                {
                    var obj = db.dvDien3PhaDinhMucs.Single(p => p.ID == (int)gvDinhMucNuoc.GetRowCellValue(item, "ID"));
                    db.dvDien3PhaDinhMucs.DeleteOnSubmit(obj);
                }
                db.SubmitChanges();
