...
{
    public partial class CtlHangMucKhongDat : DevExpress.XtraEditors.XtraUserControl
    {
        public CtlHangMucKhongDat()
        {
            InitializeComponent();
        }
        ...
        private void LoadData()
        {
            try
            {
                ...
                {
                    var checklistError = (from p in db.ho_ScheduleApartmentCheckLists
                            where ltToaNha.Contains(p.BuildingId.ToString()) &
                                  SqlMethods.DateDiffDay(tuNgay, p.DateAction) >= 0 &
                                  SqlMethods.DateDiffDay(p.DateAction, denNgay) >= 0 & p.IsLocal == true
                            group new {p} by new {p.IsNoQuality}
                            into g
                            select new {Name = g.Key.IsNoQuality == true ? "Không đạt" : "Đạt", Count = g.Count()})
                        .ToList();
