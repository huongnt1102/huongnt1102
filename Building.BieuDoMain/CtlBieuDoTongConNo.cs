...
{
    public partial class CtlBieuDoTongConNo : DevExpress.XtraEditors.XtraUserControl
    {
        /// <summary>
        /// Ý tưởng:
        /// Hôm trước mình nghĩ gì mà lại ghi là biểu đồ tổng phải thu nhỉ? Tổng phải thu là 1 con số, nếu nói danh sách tổng phải thu, thì còn có thể hiểu.
        /// Kiểu như lấy ra hết danh sách phải thu, tổng tiền phải thu. Còn biểu đồ, không lẽ làm biểu đồ so sánh các tháng? Làm thì không khó, nhưng mà nó chạy chậm lắm luôn, với không chính xác. Ví dụ trong tháng đó phải thu là phải thu trong tháng, nhưng còn cộng dồn nữa. Hay là phải thu theo tòa nhà?
        /// Chắc chổ này làm theo phải thu của từng loại dịch vụ
        /// phải thu = phát sinh + nợ đầu kỳ
        /// </summary>
        public CtlBieuDoTongConNo()
        {
            InitializeComponent();
        }
        ...
        private System.Collections.Generic.List<ChartData> GetChartDatas(System.Collections.Generic.List<SoLieu> noDauKyHd, System.Collections.Generic.List<SoLieu> noDauKySq, System.Collections.Generic.List<SoLieu> phatSinh, System.Collections.Generic.List<SoLieu> daThu, System.Collections.Generic.List<LoaiDichVu> dv)
        {
            try
            {
                return (from ldv in dv
                            ...
                            join dt in daThu on ldv.ID equals dt.MaLdv into daThus from dt in daThus.DefaultIfEmpty()
                            select new ChartData
                            {
                                StatusName = ldv.TenLDV,
                                Count = (hd != null ? hd.SoTien.GetValueOrDefault() : 0) - (sq != null ? sq.SoTien.GetValueOrDefault() : 0) + (ps != null ? ps.SoTien.GetValueOrDefault() : 0) - (dt!=null? dt.SoTien.GetValueOrDefault():0)
                            }).ToList();
            }
        ...
        private async void LoadData()
        {
            try
            {
                ...
                // data
                await System.Threading.Tasks.Task.Run(() => { chartDatas = GetChartDatas(noDauKyHd, noDauKySq, phatSinhs, daThu, loaiDichVus); });
                ...
                chartControl1.Titles[0].Text = "Tổng còn nợ: " + string.Format("{0:n0}", data.Sum(_ => _.Count)) ;

                // tạo series
                var series = new DevExpress.XtraCharts.Series("Tổng còn nợ", DevExpress.XtraCharts.ViewType.Doughnut);
                series.Points.Clear();
