...
{
    public partial class CtlDanhSachYeuCauTreHanCaNhan : DevExpress.XtraEditors.XtraUserControl
    {
        private Library.MasterDataContext _db = new Library.MasterDataContext();
        public CtlDanhSachYeuCauTreHanCaNhan()
        {
            InitializeComponent();
        }
        ...
        private System.Collections.Generic.List<YeuCauTreHan> GetYeuCauTreHans(string[] ltToaNha)
        {
            return (from yc in _db.tnycYeuCaus
                    ...
                    where ltToaNha.Contains(yc.MaTN.ToString()) & yc.NgayYC != null & yc.NgayYC.Value <= System.DateTime.UtcNow.AddHours(7) & yc.MaTT != 3 & yc.MaTT != 5 & yc.MaMB != null & yc.MaNTN == Library.Common.User.MaNV
        ...
        private async void LoadData()
        {
            try
            {
                Library.PhanQuyenBieuDo.SaveControl(GetType().Namespace + "." + Name, "Danh sách yêu cầu trễ hạn cá nhân", GetType().Namespace + ".dll");
        ...
        {
            using (var frm = new DichVu.YeuCau.frmCongViecNhanVien { MaYC = (int?)gv.GetFocusedRowCellValue("ID") }) frm.ShowDialog();
        }
    }
}
