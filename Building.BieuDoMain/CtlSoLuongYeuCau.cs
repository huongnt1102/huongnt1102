...
{
    public partial class CtlSoLuongYeuCau : DevExpress.XtraEditors.XtraUserControl
    {
        public CtlSoLuongYeuCau()
        {
            InitializeComponent();
        }

        private void CtlSoLuongYeuCau_Load(object sender, EventArgs e)
        {
        ...
        private void LoadData()
        {
            try
            {
                ...
                using (var db = new MasterDataContext())
                {
                    ...
                    Series series1 = new Series("Số lượng yêu cầu", ViewType.Pie);
                    series1.Points.Clear();

                    var tong = (double) obj.Sum(_ => _.SoLuong);
                    var hoanThanh = (double) obj.Where(_ => _.MaTrangThai == 1).Sum(_ => _.SoLuong);

                    series1.Points.Add(new SeriesPoint("Tổng", new double[] {tong}));
                    series1.Points.Add(new SeriesPoint("Yêu cầu mới", new double[] { hoanThanh}));
                    //series1.Points.Add(new SeriesPoint("Yêu cầu mới", new double[] {hoanThanh / tong}));
