...
{
    public partial class FrmBcThuChiTmDc : XtraForm
    {
        public FrmBcThuChiTmDc()
        {
            InitializeComponent();
        }
        ...
        private void LoadData()
        {
            ...
            try
            {
                ...
                if (itemTuNgay.EditValue != null && itemDenNgay.EditValue != null)
                {
                    ...
                    data.Columns.Add("Thu",typeof(decimal));
                    data.Columns.Add("Chi", typeof(decimal));
                    data.Columns.Add("TonQuy", typeof(decimal));
                    ...
                    r1["TonQuy"] = soTien;//string.Format("{0:#,0.##}", soTien);
                    r1["Key"] = 1;
                    data.Rows.Add(r1);
                    ...
                    var objTrongKy1 = (from _ in db.SoQuy_ThuChis
                        ...
                        where SqlMethods.DateDiffDay(tuNgay, _.NgayPhieu) >= 0 &
                              ...
                              _.IsKhauTru == false & _.MaLoaiPhieu == 24
                              orderby _.NgayPhieu
                        select new //BcThuChiTm
                        {
                            NgayHachToan = _.NgayPhieu, SoPhieuThu = _.IsPhieuThu == true ? _.SoPhieu : "",
                            SoPhieuChi = _.IsPhieuThu == false ? _.SoPhieu : "", MaKhachHang = kh.KyHieu, Name = kh.IsCaNhan == true?kh.HoKH+" "+kh.TenKH : kh.CtyTen, DienGiai=_.DienGiai, NhanVien = nv.HoTenNV, Thu = _.IsPhieuThu == true? _.DaThu.GetValueOrDefault():0, Chi = _.IsPhieuThu==false?_.DaThu.GetValueOrDefault():0
                        }).ToList();
                    var objTrongKy = (from _ in objTrongKy1 where _.Thu != 0 || _.Chi != 0 select _).ToList();
                    ...
                    foreach (var item in objTrongKy)
                    {
                        ...
                        r["Thu"] = item.Thu; //string.Format("{0:#,0.##}", item.Thu);
                        r["Chi"] = item.Chi; //string.Format("{0:#,0.##}", item.Chi);
                        r["TonQuy"] = soTien;//string.Format("{0:#,0.##}", soTien);
                    ...
                    r2["Thu"] = objTrongKy.Sum(_ => _.Thu);// string.Format("{0:#,0.##}", objTrongKy.Sum(_ => _.Thu));
                    r2["Chi"] = objTrongKy.Sum(_ => _.Chi);//string.Format("{0:#,0.##}", objTrongKy.Sum(_ => _.Chi));
                    r2["TonQuy"] = soTien;//string.Format("{0:#,0.##}", soTien);
                    r2["Key"] = 1;
                    data.Rows.Add(r2);
                ...
                foreach (var employee in objManagerEmployee)
                {
                    ...
                    if (objFrom != null)
                    {
                        try
                        {
                            ...
                            objMail.Subject = "Sổ kế toán chi tiết Quỹ tiền mặt đặt cọc";
                            objMail.Content = "";
                            var fileAttach = new System.Net.Mail.Attachment(result, "SoKeToanChiTietQuyTienMatDatCoc",
