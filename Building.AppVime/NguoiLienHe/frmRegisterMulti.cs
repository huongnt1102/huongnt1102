    ...
    public partial class frmRegisterMulti : DevExpress.XtraEditors.XtraForm
    {
        ...
        private void btnAccept_Click(object sender, EventArgs e)
        {
            ...
            if (IsTower)
            {
                ...
                {
                    ListResident = (from nlh in db.tnKhachHang_NguoiLienHes
            ...
            foreach (var item in ListResident)
            {
                using (var db = new MasterDataContext())
                {
                    ...
                    try
                    {
                        ...
                        var khachHang = db.tnKhachHang_NguoiLienHes.FirstOrDefault(_ => _.ID == item.ContactId);
                        if (khachHang == null)
                        {
                            continue;
                        }
                        var tn = db.tnToaNhas.FirstOrDefault(_ => _.MaTN == item.ToweId);
                        if (tn == null)
                        {
                            continue;
                        }
                        ...
                        else
                        {
                            ...
                            {
                                objRe.IsResident = false;
                                objRe.IsContact = true;
                                objRe.ContactId = item.ContactId;
                                if (objRe.Password == null) objRe.Password = pass;
                                objRe.IsLock = false;
