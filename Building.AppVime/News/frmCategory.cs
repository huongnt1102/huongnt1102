    ...
    public partial class frmCategory : DevExpress.XtraEditors.XtraForm
    {
        ...
        private void itemXoa_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            if (gvRule.FocusedRowHandle >= 0)
            {
                ...
                var count = db.app_News.FirstOrDefault(p => p.CategoryId == (int)gvRule.GetFocusedRowCellValue("Id"));
                if (count == null)
                {
                    var obj = db.app_NewsCategories.FirstOrDefault(p => p.Id == (int)gvRule.GetFocusedRowCellValue("Id"));
                    try
                    {
                        db.app_NewsCategories.DeleteOnSubmit(obj);
                        db.SubmitChanges();
        ...
        void LoadData()
        {
            ...
            gcRule.DataSource = db.app_NewsCategories;
        }
