...
{
    public partial class FrmScheduleStatus : DevExpress.XtraEditors.XtraForm
    {
        private MasterDataContext _db = new MasterDataContext();
        public FrmScheduleStatus()
        {
            InitializeComponent();
        }

        private void FrmScheduleStatus_Load(object sender, EventArgs e)
        {
        ...
        private void LoadData()
        {
            try
            {
                gc.DataSource = _db.ho_ScheduleStatus.Where(_ => _.IsStatus == true);
            }
            catch { }
        }
        ...
        {
            gv.AddNewRow();
            gv.SetFocusedRowCellValue("IsStatus", true);
        }
    }
}