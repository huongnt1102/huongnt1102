...
namespace MatBang
{
    public partial class frmMatBang : DevExpress.XtraEditors.XtraForm
    {
        ...
        void LoadData()
        {
            ...
            //Library.DialogBox.Alert("Start");

            int MaKN = (int)itemKhoiNha.EditValue;

            DataTable tblTL = SqlCommon.getData(
                  "select mb.MaTL, mb.SoNha, tl.TenTL, count(MaSoMB) as SL from mbMatBang mb "
                + "inner join mbTangLau tl on tl.MaTL=mb.MaTL "
                + "where tl.MaKN=" + MaKN
                + " group by mb.MaTL, mb.SoNha, tl.TenTL, tl.STT order by tl.STT");

            //foreach (DataRow row in tblTL.Rows)
            //{
            //    string columnvalue = row["SoNha"].ToString();
            //    Library.DialogBox.Alert(columnvalue);
            //}

                //gcMatBang.DataSource = tblTL;
            ...
            try
            {
                tblMB = SqlCommon.getData(
                    "select SoNha, MaMB, MaSoMB, mb.MaTT, mb.DienTich, mb.GiaThue as DonGia, mb.DienTich * mb.GiaThue as ThanhTien, lt.KyHieuLT as TenTG, mb.MaTL, " +
                    "TenTL, TenTT, tt.MauNen from mbMatBang mb "
                    + "left join mbTrangThai tt on tt.MaTT=mb.MaTT "
                    + "left join mbTangLau tl on tl.MaTL=mb.MaTL "
                    + "left join LoaiTien lt on lt.ID=mb.MaLT "
                    + "where tl.MaKN=" + MaKN);
                //foreach (DataRow row in tblMB.Rows)
                //{
                //    string columnvalue = row["MaSoMB"].ToString();
                //    Library.DialogBox.Alert(columnvalue);
                //}
                ...
                foreach (var item in coltl)
                {
                    ...
                    //Library.DialogBox.Alert(colMaSo.FieldName);
                }
                ...
                for (int i = 0; i < tblTL.Rows.Count; i++)
                {
                    ...
                    for (int j = 0; j < rMB.Length; j++)
                    {
                        ...
                        var index = coltl.FirstOrDefault(p => p.MaTL.ToString() == tblTL.Rows[i]["MaTL"].ToString())
                            .MaTL.ToString();
                        tbl.Rows[j][index] = rMB[j]["MaSoMB"].ToString();

                        //Library.DialogBox.Alert(rMB[j]["MaSoMB"].ToString());
                    }
                }
