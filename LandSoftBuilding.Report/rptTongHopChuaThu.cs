...
{
    public partial class rptTongHopChuaThu : DevExpress.XtraReports.UI.XtraReport
    {
        public rptTongHopChuaThu(int month, int year, int maTN)
        {
            InitializeComponent();
            ...
            try
            {
                ...
                this.DataSource = (from hd in db.dvHoaDons
                                   ...
                                   select new
                                   {
                                       gr.Key.MaSoMB,
                                       gr.Key.TenKH,
                                       PQL = gr.Sum(p => p.MaLDV == (int)MaLDVs.PQL & SqlMethods.DateDiffMonth(p.NgayTT, _Ngay) == 0 ?
                                           (p.PhaiThu -
                                                (from ct in db.ptChiTietPhieuThus
                                                 join pt in db.ptPhieuThus on ct.MaPT equals pt.ID
                                                 where ct.TableName == "dvHoaDon" & ct.LinkID == p.ID & SqlMethods.DateDiffMonth(pt.NgayThu, _Ngay) >= 0
                                                 select ct.SoTien).Sum().GetValueOrDefault()) 
                                           : 0).GetValueOrDefault(),
                                       Nuoc = gr.Sum(p => p.MaLDV == (int)MaLDVs.Nuoc & SqlMethods.DateDiffMonth(p.NgayTT, _Ngay) == 0 ?
                                           (p.PhaiThu -
                                                (from ct in db.ptChiTietPhieuThus
                                                 join pt in db.ptPhieuThus on ct.MaPT equals pt.ID
                                                 where ct.TableName == "dvHoaDon" & ct.LinkID == p.ID & SqlMethods.DateDiffMonth(pt.NgayThu, _Ngay) >= 0
                                                 select ct.SoTien).Sum().GetValueOrDefault()) 
                                           : 0).GetValueOrDefault(),
                                       Gas = gr.Sum(p => p.MaLDV == (int)MaLDVs.Gas & SqlMethods.DateDiffMonth(p.NgayTT, _Ngay) == 0 ?
                                           (p.PhaiThu -
                                                (from ct in db.ptChiTietPhieuThus
                                                 join pt in db.ptPhieuThus on ct.MaPT equals pt.ID
                                                 where ct.TableName == "dvHoaDon" & ct.LinkID == p.ID & SqlMethods.DateDiffMonth(pt.NgayThu, _Ngay) >= 0
                                                 select ct.SoTien).Sum().GetValueOrDefault()) 
                                           : 0).GetValueOrDefault(),
                                       PGX = gr.Sum(p => p.MaLDV == (int)MaLDVs.PGX & SqlMethods.DateDiffMonth(p.NgayTT, _Ngay) == 0 ?
                                           (p.PhaiThu -
                                                (from ct in db.ptChiTietPhieuThus
                                                 join pt in db.ptPhieuThus on ct.MaPT equals pt.ID
                                                 where ct.TableName == "dvHoaDon" & ct.LinkID == p.ID & SqlMethods.DateDiffMonth(pt.NgayThu, _Ngay) >= 0
                                                 select ct.SoTien).Sum().GetValueOrDefault()) 
                                           : 0).GetValueOrDefault(),
                                       PVS = gr.Sum(p => p.MaLDV == (int)MaLDVs.PVS & SqlMethods.DateDiffMonth(p.NgayTT, _Ngay) == 0 ?
                                           (p.PhaiThu -
                                                (from ct in db.ptChiTietPhieuThus
                                                 join pt in db.ptPhieuThus on ct.MaPT equals pt.ID
                                                 where ct.TableName == "dvHoaDon" & ct.LinkID == p.ID & SqlMethods.DateDiffMonth(pt.NgayThu, _Ngay) >= 0
