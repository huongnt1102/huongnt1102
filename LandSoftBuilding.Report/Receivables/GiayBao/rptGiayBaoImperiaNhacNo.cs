...
{
    public partial class rptGiayBaoImperiaNhacNo : DevExpress.XtraReports.UI.XtraReport
    {
        byte MaTN;
        int MaKH, _Thang, _Nam;

        public rptGiayBaoImperiaNhacNo(byte _MaTN, int _Thang, int _Nam, int _MaKH, List<int> _MaLDVs, int _MaTKNH)
        {
            InitializeComponent();
            ...
            try
            {
                Library.frmPrintControl.LoadLayout(this, 92, _MaTN);
                ...
                var tam = (from hd in db.dvHoaDons
                           join ldv in db.dvLoaiDichVus on hd.MaLDV equals ldv.ID
                                   ...
                                   group hd by new { hd.MaLDV, ldv.TenLDV, hd.KyTT, ldv.NgayHanTT} into gr
