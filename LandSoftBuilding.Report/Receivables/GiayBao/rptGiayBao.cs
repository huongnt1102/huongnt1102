...
namespace LandSoftBuilding.Receivables.GiayBao
{
    public partial class rptGiayBao : DevExpress.XtraReports.UI.XtraReport
    {
        ...
        public rptGiayBao(byte _MaTN, int _Thang, int _Nam, int _MaKH,int _MaMB, List<int> _MaLDVs, int _MaTKNH)
        {
            InitializeComponent();
            ...
            try
            {
                ...
                var objKH = (from mb in db.mbMatBangs
                             ...
                             select new
                             {
                                 ...
                                 ThuTruoc = db.SoQuy_ThuChis.Where(p => p.IsPhieuThu == true && p.MaKH == (int?)_MaKH).Sum(s => s.ThuThua.GetValueOrDefault() - s.KhauTru.GetValueOrDefault()),
                             }).First();
