...
{
    public partial class rptGiayBaoImperia : DevExpress.XtraReports.UI.XtraReport
    {
        byte MaTN;
        int MaKH, Thang, Nam;

        public rptGiayBaoImperia(byte _MaTN, int _Thang, int _Nam, int _MaKH, List<int> _MaLDVs, int _MaTKNH)
        {
            InitializeComponent();
            ...
            try
            {
                Library.frmPrintControl.LoadLayout(this, 96, _MaTN);
                ...
                var ltLoaiDichVu = (from hd in db.dvHoaDons
                                    ...
                                    & hd.NgayTT.Value.Year == _Nam
                                     & (hd.MaLDV == 9 | hd.MaLDV == 6 | hd.MaLDV == 8)
                ...
                var _TongTien = Convert.ToInt64(Math.Round(_PhatSinh - _ThuThua + _NoCu , 0));
               
                csumTongTien.Text = string.Format("{0:#,0}", _TongTien > 0 ? _TongTien : 0);
                string lama = "";
                for (int i = 1; i <= ltData.Count(); i++)
                {
                    lama += RomanNumerals.ToRoman(i)+ "+";
                }
                if(lama!="")
                lama = lama.TrimEnd('+');
                xrTableCell1.Text = lama;
            }
            catch { }
            finally
            {
            ...
            try
            {
                ...
                switch (_MaLDV)
                {
                    ...
                    case 5:
                        ...
                        break;
                    case 6:
                        rptDichVu.ReportSource = new rptTheXeImperia(this.MaTN, this.MaKH, this.Thang, this.Nam, RomanNumerals.ToRoman(STT));
                        break;
                    case 8:
                        rptDichVu.ReportSource = new rptTienDienImperia(this.MaTN, this.MaKH, this.Thang, this.Nam, RomanNumerals.ToRoman(STT));
                        break;
                    case 9:
                        rptDichVu.ReportSource = new rptTienNuocImperia(this.MaTN, this.MaKH, this.Thang, this.Nam, RomanNumerals.ToRoman(STT));
                        break;
                    case 10:
