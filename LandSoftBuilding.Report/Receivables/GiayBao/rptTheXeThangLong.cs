...
{
    public partial class rptTheXeThangLong : DevExpress.XtraReports.UI.XtraReport
    {
        public rptTheXeThangLong(byte _MaTN, int _MaKH, int _Thang, int _Nam)
        {
            InitializeComponent();
                ...
                if (_MaTN != 27)
                {
                    var ltGiuXete =(from tx in db.dvgxTheXes
                                  ...
                                  // join gx in db.dvgxGiuXes on tx.MaGX equals gx.ID
                                   ...
                                   where tx.MaTN == _MaTN & tx.MaKH == _MaKH & tx.NgungSuDung == false & dv.NgayTT.Value.Month == _Thang & dv.NgayTT.Value.Year == _Nam 
                                   //& (pt.DaThu == null)
                                   & dv.IsDuyet == true
                                   & (dv.PhaiThu.GetValueOrDefault() - (db.ptChiTietPhieuThus.Where(p => p.LinkID == dv.ID & p.TableName == "dvHoaDon").Sum(p => p.SoTien)).GetValueOrDefault()) > 0
                                   group tx by new { tx.MaLX, lx.TenLX, tx.GiaThang } into gr
                                   select new GiuXeItem1()
                                   {
                                       MaLX = gr.Key.MaLX,
                                       TenLX = gr.Key.TenLX,
                                       BienSo = "",
