...
{
    public partial class rptTienDienTTC_XemLai : DevExpress.XtraReports.UI.XtraReport
    {
        public rptTienDienTTC_XemLai(byte _MaTN, int _MaKH, int _Thang, int _Nam)
        {
            InitializeComponent();
            ...
            try
            {
               var tam = (from hd in db.dvHoaDons
                                   ...
                                   where hd.MaTN == _MaTN & hd.MaKH == _MaKH & hd.NgayTT.Value.Month == _Thang & hd.NgayTT.Value.Year == _Nam
                                    & (hd.PhaiThu.GetValueOrDefault() -(db.SoQuy_ThuChis.Where(p => p.LinkID == hd.ID & p.TableName == "dvHoaDon" & SqlMethods.DateDiffDay(p.NgayPhieu, new DateTime(_Nam, _Thang, 1)) > 0).Sum(p => p.DaThu + p.KhauTru)).GetValueOrDefault()) > 0
