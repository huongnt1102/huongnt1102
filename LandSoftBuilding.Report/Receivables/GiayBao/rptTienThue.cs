...
{
    public partial class rptTienThue : DevExpress.XtraReports.UI.XtraReport
    {
        public rptTienThue(byte _MaTN, int _MaKH, int _Thang, int _Nam)
        {
            InitializeComponent();
            ...
            try
            {
                this.DataSource = (from hd in db.dvHoaDons
                                   ...
                                   where hd.MaTN == _MaTN & hd.MaLDV == 2 & hd.MaKH == _MaKH & hd.NgayTT.Value.Month == _Thang & hd.NgayTT.Value.Year == _Nam & hd.ConNo.GetValueOrDefault() > 0
                                   select new
                                   {
                                       ViTri = tl.TenTL,
                                       ltt.TuNgay,
