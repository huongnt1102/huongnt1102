...
{
    public partial class rptGiayBaoImperiaPQL : DevExpress.XtraReports.UI.XtraReport
    {
        byte MaTN;
        int MaKH, Thang, Nam;

        public rptGiayBaoImperiaPQL(byte _MaTN, int _Thang, int _Nam, int _MaKH, List<int> _MaLDVs, int _MaTKNH)
        {
            InitializeComponent();
            ...
            try
            {
                Library.frmPrintControl.LoadLayout(this, 93, _MaTN);
                ...
                string Quy = "";
                if (this.Thang >= 1 & this.Thang <= 3)
                {
                    Quy = "Qúy I";
                }
                if (this.Thang >= 4 & this.Thang <= 6)
                {
                    Quy = "Qúy II";
                }
                if (this.Thang >= 7 & this.Thang <= 9)
                {
                    Quy = "Qúy III";
                }
                if (this.Thang >= 10 & this.Thang <= 12)
                {
                    Quy = "Qúy IV";
                }
                ...
                ctlRTF.Document.ReplaceAll("[Quy]", Quy ?? "", SearchOptions.None);
                ...
                var ltLoaiDichVu = (from hd in db.dvHoaDons
                                    ...
                                    & hd.NgayTT.Value.Year == _Nam & hd.MaLDV == 13
                                    ...
                                    - (db.ptChiTietPhieuThus.Where(p => p.LinkID == hd.ID & p.TableName == "dvHoaDon").Sum(p => p.SoTien)).GetValueOrDefault())
                                       - (db.ktttChiTiets.Where(p => p.LinkID == hd.ID & p.TableName == "dvHoaDon").Sum(p => p.SoTien)).GetValueOrDefault() > 0  //moi mo 5/11/2016
                ...
                var ltData = (from hd in db.dvHoaDons
                              ...
                              group hd by new { hd.MaLDV, ldv.TenHienThi, ldv.TenTA, hd.KyTT } into gr
                              select new
                              {
                                  gr.Key.MaLDV,
                                  TenLDV = gr.Key.TenHienThi,
            ...
            try
            {
                ...
                switch (_MaLDV)
                {
                    ...
                    case 5:
                        ...
                        break;
                    case 6:
                        rptDichVu.ReportSource = new rptTheXeImperia(this.MaTN, this.MaKH, this.Thang, this.Nam, "");
                        break;
                    case 8:
        ...
        private void rtTenLDV_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            ...
            {
                var _MaLDV = (int?)GetCurrentColumnValue("MaLDV");
                var KyTT = Convert.ToInt32(GetCurrentColumnValue("KyTT"));
                ...
                STT++;
                rtTenLDV.Html = KyTT == 3 ?
                     string.Format("<div style='font-family:Times New Roman; font-size:9pt'><b>{0}. {1} <i>/ {2}</i> ({3:00}/{4})</b></div>",
                    RomanNumerals.ToRoman(STT), GetCurrentColumnValue("TenLDV"),
                    GetCurrentColumnValue("TenTA"),
                    Quy, _Nam)


                    : string.Format("<div style='font-family:Times New Roman; font-size:9pt'><b>{0}. {1} <i>/ {2}</i> (Tháng {3:00}/{4})</b></div>",
                    RomanNumerals.ToRoman(STT), GetCurrentColumnValue("TenLDV"),
                    GetCurrentColumnValue("TenTA"),
                    _ThangDV, _Nam);
            }
