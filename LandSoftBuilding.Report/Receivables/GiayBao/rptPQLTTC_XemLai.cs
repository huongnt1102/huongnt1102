...
{
    public partial class rptPQLTTC_XemLai : DevExpress.XtraReports.UI.XtraReport
    {
        public rptPQLTTC_XemLai(byte _MaTN, int _MaKH, int _Thang, int _Nam, int _MaLDV)
        {
            InitializeComponent();
            ...
            try
            {
               var tam = (from hd in db.dvHoaDons
                                   ...
                                   where hd.MaTN == _MaTN & hd.MaLDV == _MaLDV & hd.MaKH == _MaKH & hd.NgayTT.Value.Month == _Thang & hd.NgayTT.Value.Year == _Nam
                                    & (hd.PhaiThu.GetValueOrDefault() - db.SoQuy_ThuChis.Where(sq => sq.LinkID == hd.LinkID && sq.TableName == "dvHoaDon" & SqlMethods.DateDiffDay(sq.NgayPhieu, new DateTime(_Nam, _Thang, 1)) > 0).Sum(s => s.DaThu + s.KhauTru).GetValueOrDefault()
                    ) > 0
                          select new
                                   {
                                       dv.SoLuong,
                ...
                var _NoCu = (from hd in db.dvHoaDons
                             where hd.MaKH == _MaKH
                              & SqlMethods.DateDiffDay(hd.NgayTT, NgayTB) > 0 & (hd.MaLDV == 13 || hd.MaLDV == 69)
                ...
                csumThanhTien.Text = string.Format("{0:#,0.##}", _NoCu + tam.Sum(p=>p.ThanhTien).GetValueOrDefault());
            }
            catch { }
            finally
            {
