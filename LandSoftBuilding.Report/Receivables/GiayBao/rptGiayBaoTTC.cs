...
{
    public partial class rptGiayBaoTTC : DevExpress.XtraReports.UI.XtraReport
    {
        byte MaTN;
        int MaKH, Thang, Nam;

        public rptGiayBaoTTC(byte _MaTN, int _Thang, int _Nam, int _MaKH, int _MaMB,List<int> _MaLDVs, int _MaTKNH)
        {
            InitializeComponent();
            ...
            try
            {
                ...
                var ltData = (from hd in db.dvHoaDons
                              ...
                              - (db.SoQuy_ThuChis.Where(p => p.LinkID == hd.ID & p.TableName == "dvHoaDon").Sum(p => p.DaThu+ p.KhauTru)).GetValueOrDefault())> 0
                              & hd.IsDuyet == true
                ...
                var _NoCu = (from hd in db.dvHoaDons
                             ...
                             select (hd.PhaiThu - (db.SoQuy_ThuChis.Where(p => p.LinkID == hd.ID & p.MaKH==_MaKH & p.TableName == "dvHoaDon").Sum(p => p.DaThu+p.KhauTru-p.ThuThua)).GetValueOrDefault())).Sum().GetValueOrDefault();
                if (_NoCu < 0)
                {
                    _NoCu = 0;
                }
            ...
            try
            {
                ...
                switch (_MaLDV)
                {
                    ...
                    case 5:
                        ...
                        break;
                    case 6:
                        rptDichVu.ReportSource = new rptTheXeTTC(this.MaTN, this.MaKH, this.Thang, this.Nam);
                        break;
                    case 8:
                        rptDichVu.ReportSource = new rptTienDienTTC(this.MaTN, this.MaKH, this.Thang, this.Nam);
                        break;
                    case 9:
                        rptDichVu.ReportSource = new rptTienNuocTTC(this.MaTN, this.MaKH, this.Thang, this.Nam);
                        break;
                    case 10:
                    ...
                    case 49: //Tien lai
                        ...
                        break;
                    case 13: //Tien lai
                        rptDichVu.ReportSource = new rptPQLTTC(this.MaTN, this.MaKH, this.Thang, this.Nam, _MaLDV.Value);
                        break;
                    default:
                   ...
                   // cTuNgayDenNgay.Text = string.Format("Từ/From    {0:dd/MM/yyyy}           Đến/to     {1:dd/MM/yyyy}", objNuoc.TuNgay, objNuoc.DenNgay);
                    ...
                    var objNuoc = (from hd in db.dvHoaDons
                                   ...
                                   select new
                                   {
                                       ...
                                       ConNo = hd.PhaiThu - db.ptChiTietPhieuThus.Where(p => p.LinkID == hd.ID).Sum(p => p.SoTien).GetValueOrDefault()
                                       - db.ktttChiTiets.Where(p => p.LinkID == hd.ID).Sum(p => p.SoTien).GetValueOrDefault()
                                   }).First();
