...
{
    public partial class rptGiayBao3 : DevExpress.XtraReports.UI.XtraReport
    {
        ...
        public rptGiayBao3(byte _MaTN, int _Thang, int _Nam, int _MaKH, List<int> _MaLDVs, int _MaTKNH)
        {
            InitializeComponent();
            ...
            try
            {
                ...
                //Load layout
                Library.frmPrintControl.LoadLayout(this, 37, _MaTN);
                ...
                var ltLoaiDichVu = (from hd in db.dvHoaDons
                                    ...
                                    select new
                                    {
                                        ...
                                        SoTien = gr.Sum(p => p.PhaiThu).GetValueOrDefault() -
                                                    (from p in db.ptChiTietPhieuThus
                                                     join h in db.dvHoaDons on p.LinkID equals h.ID
                                                     join l in db.dvLoaiDichVus on h.MaLDV equals l.ID
                                                     where h.IsDuyet == true & h.MaKH == _MaKH & h.NgayTT.Value.Month == _Thang & h.NgayTT.Value.Year == _Nam 
                                                     & h.MaLDV == gr.Key.MaLDV
                                                     & (h.PhaiThu.GetValueOrDefault() - (db.ptChiTietPhieuThus.Where(o => o.LinkID == h.ID & p.TableName == "dvHoaDon").Sum(o => o.SoTien)).GetValueOrDefault()) > 0
                                                     select p.SoTien
                                            ).Sum().GetValueOrDefault(),
                                    }).ToList();

                var ltNoCu = (from hd in db.dvHoaDons
                              join ldv in db.dvLoaiDichVus on hd.MaLDV equals ldv.ID
                              where hd.MaLDV != 23 & hd.IsDuyet == true & hd.MaKH == _MaKH & SqlMethods.DateDiffDay(hd.NgayTT, new DateTime(_Nam, _Thang, 1)) > 0
                                //& (_MaLDVs.Contains(hd.MaLDV.Value) | _MaLDVs.Count == 0)
                                & (hd.PhaiThu.GetValueOrDefault() - (db.ptChiTietPhieuThus.Where(p => p.LinkID == hd.ID & p.TableName == "dvHoaDon").Sum(p => p.SoTien)).GetValueOrDefault()) > 0
                              group hd by new { ldv.STT, hd.MaLDV, ldv.TenHienThi, ldv.TenTA } into gr
                              orderby gr.Key.STT
                              select new
                              {
                                  ...
                                  SoTien = gr.Sum(p => p.PhaiThu).GetValueOrDefault() - 
                                            (from p in db.ptChiTietPhieuThus  
                                             join h in db.dvHoaDons on p.LinkID equals h.ID
                                             join l in db.dvLoaiDichVus on h.MaLDV equals l.ID
                                             where h.IsDuyet == true & h.MaKH == _MaKH & SqlMethods.DateDiffDay(h.NgayTT, new DateTime(_Nam, _Thang, 1)) > 0
                                             & h.MaLDV == gr.Key.MaLDV
                                             & ( h.PhaiThu.GetValueOrDefault() - (db.ptChiTietPhieuThus.Where(o => o.LinkID == h.ID & p.TableName == "dvHoaDon").Sum(o => o.SoTien)).GetValueOrDefault()) > 0
                                             select p.SoTien
                                            ).Sum().GetValueOrDefault(),
                ...
                var _PhatSinh = ltLoaiDichVu.Sum(p => p.SoTien);
                var _NoCu = ltNoCu.Sum(p => p.SoTien);
                ...
                csumTongTien.Text = string.Format("{0:#,0.##}", _TongTien);
                ...
                objGB.TenCongTy = objTN.CongTyQuanLy;
                objGB.TenToaNha = objTN.TenTN;
                objGB.MaSoMB = objKH.MaSoMB;
                objGB.TenTL = objKH.TenTL;
                objGB.TenKN = objKH.TenKN;
                objGB.MaSoKH = objKH.KyHieu;
                objGB.MaPhuKH = objKH.MaPhu;
                objGB.TenKH = objKH.TenKH;
                objGB.DiaChiKH = objKH.DCLL;
                ...
                if (ltNoCu.Count > 0)
                {
                    ...
                    var objLDV = new DichVuItem();
                    objLDV.STT = RomanNumerals.ToRoman(_SoLaMa);
                    objLDV.TenLDV = "Nợ kỳ trước chưa thanh toán";
                    objLDV.ThanhTien = _NoCu;
                    objLDV.Details = new List<DichVuItem>();
                    foreach (var ldv in ltNoCu)
                    {
                        var objDetail = new DichVuItem();
                        objDetail.TenLDV = string.Format("{0}/{1}", ldv.TenLDV, ldv.TenTA);
                        objDetail.ThanhTien = ldv.SoTien;
                        objLDV.Details.Add(objDetail);
                    }
