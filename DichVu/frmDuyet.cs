...
namespace DichVu
{
    public partial class frmDuyet : XtraForm
    {
        ...
        private void btnSave_Click(object sender, System.EventArgs e)
        {
            using (var db = new MasterDataContext())
            {
                try
                {
                    ...
                    //var idNew = 17;
                    var body = new VanChuyenModel();
                    body.traLoi = meNoiDung.Text;
                    body.manvDuyet = Common.User.MaNV;
                    body.Duyet = checkDuyet.Checked;
                    var post = VimeService.PostH(body, $"/Delivery/Duyet?idNew={idNew}&idPhieuVanChuyen={ID}");
                    this.Close();
                }
                catch (Exception ex)
                {
        ...
        private void frmDuyet_Load(object sender, EventArgs e)
        {
            ...
            if (check != null)
            {
                meNoiDung.Text = check.LyDoKhongDuyet;
