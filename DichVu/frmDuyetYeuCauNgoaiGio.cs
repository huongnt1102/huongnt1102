...
namespace DichVu
{
    public partial class frmDuyetYeuCauNgoaiGio : XtraForm
    {
        MasterDataContext db = new MasterDataContext();
        public int? IDToaNha { get; set; }
        public int? ID { get; set; }
        public frmDuyetYeuCauNgoaiGio()
        {
            InitializeComponent();
        }
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            using (var db = new MasterDataContext())
            {
                try
                {
                    ...
                    var post = VimeService.PostH(body, $"/LamNgoaiGio/Duyet?idNew={idNew}&idYeuCau={ID}");
                    this.Close();
                }
                catch (Exception ex)
                {
        ...
        private void frmDuyetYeuCauNgoaiGio_Load(object sender, EventArgs e)
        {
            var check = db.tnLamViecNgoaiGios.SingleOrDefault(p => p.ID == ID);
            if (check != null)
            {
                meNoiDung.Text = check.TraLoi;
                checkDuyet.Checked = (bool)check.Duyet;
            }
        }
    }
}
