...
namespace Building.ServiceAndUtilities.KhachHang
{
    public partial class frmAddAll : DevExpress.XtraEditors.XtraForm
    {
        ...
        private void frmEdit_Load(object sender, EventArgs e)
        {
            ...
            chkKhachHang.Properties.DataSource = db.tnKhachHangs.Where(_=>_.MaTN == MaTN).Select(_=>new {_.MaKH, TenKH = _.IsCaNhan == true ? _.TenKH : _.CtyTen}).ToList();
            //if(Id != 0)
            //{
        ...
        private void itemLuu_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            var strMatBang = (chkKhachHang.EditValue ?? "").ToString().TrimEnd(',').TrimStart(',').Replace(" ", "");
            ...
            {
                var model = new { id = Id, makh = int.Parse(item), keHoachId = KeHoachId, nguoiLienHe = txtNguoiDaiDien.Text, soDienThoai = txtSoDienThoai.Text, chucVu = txtChucVu.Text, tuNgay = dateTuNgay.DateTime, denNgay = dateDenNgay.DateTime, ghiChu = txtGhiChu.Text, isXacNhan = IsXacNhan };
