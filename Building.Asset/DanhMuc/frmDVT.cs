...
{
    public partial class frmDVT : XtraForm
    {
        ...
        private void LoadData()
        {
            ...
            gridControl.DataSource = _db.tbl_VatTu_DVTs;
        }
        ...
        private void barButtonItemXoa_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            try
            {
                var obj = _db.tbl_VatTu_DVTs.FirstOrDefault(_ => _.ID == (int) gridView.GetFocusedRowCellValue("ID"));
                if (obj != null)
                {
                    _db.tbl_VatTu_DVTs.DeleteOnSubmit(obj);
                }
        ...
        {
            var tenVietTat = gridView.GetFocusedRowCellValue("TenDVT");
            if (tenVietTat == null)
            {
            ...
            if (IsDuplication("TenDVT", e.RowHandle, tenVietTat.ToString().ToLower()))
            {
                e.ErrorText = "Tên DVT này đã có";
                e.Valid = false;
