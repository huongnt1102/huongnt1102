...
{
    public partial class frmBieuDo_ThongKeSuDungCacToaNha : Form
    {
        public frmBieuDo_ThongKeSuDungCacToaNha()
        {
            InitializeComponent();
        }
            ...
            try
            {
                ...
                using (var db = new MasterDataContext())
                {
                    ...
                    var ltSource = (from news in db.app_Residents
                        ...
                        where //tower.MaTN == maTn & 
                              ltToaNha.Contains(tower.MaTN.ToString()) &
                              SqlMethods.DateDiffDay(tuNgay, news.DateOfCreate) >= 0 &
                              SqlMethods.DateDiffDay(news.DateOfCreate, denNgay) >= 0 &
                              news.IsResident == true
                        group new {news,tower} by new
                            {tower.MaTN,tower.TenTN}
                        into g
                        orderby g.Key.TenTN
                        select new
                        {
                            TenTN = g.Key.TenTN, Login = g.Sum(_ => _.news.IsLogin != null ? 1 : 0),Count=g.Count()
                        }).ToList();
                    ...
                    {
                        series1.Points.Add(new SeriesPoint("T" + i.TenTN, i.Count));
                        series2.Points.Add(new SeriesPoint("T" + i.TenTN, i.Login));
                    }
                    ...
                    var strTitle = string.Format("THỐNG KÊ ĐĂNG KÝ/ SỬ DỤNG APP CỦA CÁC TÒA NHÀ \n TỪ {0:MM/yyyy} ĐẾN {1:MM/yyyy}", tuNgay, denNgay);
