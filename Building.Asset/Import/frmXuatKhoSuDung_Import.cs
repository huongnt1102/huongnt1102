...
{
    public partial class frmXuatKhoSuDung_Import : XtraForm
    {
        public byte MaTn { get; set; }
        public bool IsSave { get; set; }
        public int LoaiXuat { get; set; }

        public frmXuatKhoSuDung_Import()
        {
            InitializeComponent();
        }
        ...
        private void itemSheet_EditValueChanged(object sender, EventArgs e)
        {
            if (itemSheet.EditValue == null)
                ...
                try
                {
                    ...
                    System.Collections.Generic.List<XuatKhoSuDung> list = Library.Import.ExcelAuto.ConvertDataTable<XuatKhoSuDung>(Library.Import.ExcelAuto.GetDataExcel(excel, gv, itemSheet));

                    gc.DataSource = list;
                    //gc.DataSource = excel.Worksheet(itemSheet.EditValue.ToString()).Select(_ => new XuatKhoSuDung
                    //{
                    //    MaPhieu = _["Mã phiếu"].ToString().Trim(),
                    //    ChiTietTaiSan = _["Chi tiết tài sản"].ToString().Trim(),
        ...
        private void itemSave_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            try
            {
                var obj = (List<XuatKhoSuDung>) gc.DataSource;
                var ltError = new List<XuatKhoSuDung>();
                foreach (var n in obj)
                {
                    try
                    {
                        ...
                        var chiTiet = db.tbl_ChiTietTaiSans.FirstOrDefault(_ =>
                            _.MaTN == MaTn & _.NgungSuDung == false &
                            _.MaChiTietTaiSan.ToLower() == n.ChiTietTaiSan.ToLower());
                        if (chiTiet == null)
                        {
                            n.Error = "Chi tiết tài sản " + n.ChiTietTaiSan + " không có trong hệ thống";
                            ltError.Add(n);
                            continue;
                        }
                        ...
                        ctXk.ChiTietTaiSanID = chiTiet.ID;
